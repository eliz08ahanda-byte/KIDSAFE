...
export const TrackerDashboard = ({ parentConfig, onLogout }) => {
  ...
  const children = React.useMemo(() => {
    if (parentConfig?.children && Array.isArray(parentConfig.children)) return parentConfig.children;
  ...
  React.useEffect(() => {
    if (!selectedChildId && children.length > 0) {
      setSelectedChildId(parentConfig?.uid || children[0].id);
    } else if (selectedChildId && !children.some((child) => child.id === selectedChildId) && children.length > 0) {
      setSelectedChildId(children[0].id);
    }
  }, [children, selectedChildId, parentConfig]);

  // LIVE COORDINATES LISTENER FROM ESP32 SLOTS
  React.useEffect(() => {
    // FORCE UPDATE: Explicitly listen to your parent account User ID string path
    const targetDocId = parentConfig?.uid || selectedChildId; 
    ...
    const unsubLive = onSnapshot(doc(db, 'parents', targetDocId), (docSnap) => {
      if (docSnap.exists()) {
        ...
        const normalizedData = {
          ...
          fence_lat: data.fence_lat ? Number(data.fence_lat) : (data.latitude ? Number(data.latitude) : 3.8834),
          fence_lng: data.fence_lng ? Number(data.fence_lng) : (data.longitude ? Number(data.longitude) : 11.5528),
    ...
    const unsubscribeHistory = onSnapshot(
      ...
      (snapshot) => {
        const events = snapshot.docs.map((docSnap) => {
          const data = docSnap.data();
          const timestamp = data.timestamp?.toDate ? data.timestamp.toDate() : data.timestamp || new Date();
          return {
            id: docSnap.id,
            ...
            notes: data.notes || '',
            timestamp,
            zoneStatus: data.zoneStatus || (data.outsideZone ? 'Outside Safe Zone' : 'Inside Safe Zone'),
          };
        });
  ...
  const selectedChild = children.find((child) => child.id === selectedChildId || child.id === parentConfig?.uid) || children[0] || {
    id: 'child_001',
    name: 'Child',
    age: 8,
    avatar: '👦',
  ...
  return (
    <div
      style={{
        ...
        backgroundImage: 'radial-gradient(circle at top left, rgba(74, 144, 226, 0.08), transparent 38%), radial-gradient(circle at bottom right, rgba(69, 139, 222, 0.06), transparent 30%)',
        minHeight: '100vh',
    ...
    >
      {isSOS && (
        <SOSAlert childName={childName} childId={parentConfig?.uid || selectedChildId} ringtone={sosRingtone} onDismiss={() => setChildData((prev) => ({ ...prev, sos: false }))} />
      )}

      {activeTab === 'home' && (
        <div>
          <DashboardHeader title="Home Dashboard" isSafe={isSafe} childName={childName} childAvatar={childAvatar} childPhotoUrl={childPhotoUrl} />
          <div style={{ padding: '20px' }}>
            <StatusCard isSafe={isSafe} childName={childName} childAge={childAge} childAvatar={childAvatar} childPhotoUrl={childPhotoUrl} />
            ...
            <div style={{ backgroundColor: 'white', padding: '22px', borderRadius: '24px', marginBottom: '22px', boxShadow: '0 16px 36px rgba(15, 23, 42, 0.06)' }}>
        ...
        <div style={{ padding: '20px' }}>
          ...
          >
            ...
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ color: '#6B7280', fontSize: '13px' }}>{`Battery ${childData?.battery || 100}%`}</div>
            </div>
          </div>

          <div style={{ display: 'grid', gap: '18px', marginBottom: '22px', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))' }}>
            <div
              style={{
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                ...
                <span>{zoneLatitude && zoneLongitude ? `${zoneLatitude.toFixed(5)}, ${zoneLongitude.toFixed(5)}` : 'Syncing...'}</span>
              </div>
          ...
          <div style={{ backgroundColor: 'white', padding: '14px', borderRadius: '20px', overflow: 'hidden', marginBottom: '20px' }}>
            <MapContainer
              center={[childData?.latitude || 3.8834, childData?.longitude || 11.5528]}
              zoom={14}
      ...
      {activeTab === 'history' && (
        <div style={{ padding: '20px' }}>
          ...
          <div style={{ display: 'grid', gap: '16px', marginBottom: '20px', gridTemplateColumns: '1fr 1fr' }}>
            <div style={{ backgroundColor: 'white', borderRadius: '20px', padding: '18px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)' }}>
              <h3 style={{ margin: '0 0 12px 0', color: '#2C3E50' }}>Recent Events</h3>
              <div style={{ fontSize: '24px', fontWeight: '700', color: '#111827' }}>{historyEvents.length}</div>
              <div style={{ marginTop: '8px', color: '#6B7280' }}>Most recent location updates available.</div>
            </div>
            <div style={{ backgroundColor: 'white', borderRadius: '20px', padding: '18px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)' }}>
              <h3 style={{ margin: '0 0 12px 0', color: '#2C3E50' }}>Latest Status</h3>
              <div style={{ color: isSafe ? '#16A34A' : '#DC2626', fontWeight: '700', fontSize: '18px' }}>
                {isSafe ? 'Currently Safe' : 'Check Zone Alert'}
              </div>
              <div style={{ marginTop: '8px', color: '#6B7280' }}>{`Current battery: ${childData?.battery || 100}%`}</div>
            </div>
          </div>

          <div style={{ backgroundColor: 'white', borderRadius: '20px', padding: '18px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)', marginBottom: '20px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              <h3 style={{ margin: 0, color: '#2C3E50' }}>Timeline</h3>
              <span style={{ color: '#6B7280', fontSize: '14px' }}>{historyLoading ? 'Loading...' : `${historyEvents.length} records`}</span>
            </div>
            {historyLoading ? (
              <p style={{ color: '#6B7280', margin: 0 }}>Fetching history events...</p>
            ) : historyEvents.length === 0 ? (
              <p style={{ color: '#6B7280', margin: 0 }}>No history data found yet. Make sure your tracker is sending updates.</p>
            ) : (
              <div style={{ display: 'grid', gap: '14px' }}>
                {historyEvents.map((event) => (
                  <div key={event.id} style={{ padding: '14px', borderRadius: '16px', backgroundColor: '#F8FAFF' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                      <div>
                        <div style={{ fontWeight: '700', color: '#111827' }}>{event.eventType}</div>
                        <div style={{ color: '#6B7280', fontSize: '13px' }}>{formatEventTime(event.timestamp)}</div>
                      </div>
                      <div style={{ color: event.zoneStatus?.includes('Outside') ? '#DC2626' : '#16A34A', fontWeight: '700' }}>
                        {event.zoneStatus}
                      </div>
                    </div>
                    <div style={{ color: '#374151', fontSize: '14px', marginBottom: '8px' }}>
                      {event.notes || `${event.latitude ? `Lat ${event.latitude}` : 'Lat N/A'}, ${event.longitude ? `Lng ${event.longitude}` : 'Lng N/A'}`}
                    </div>
          ...
          <div style={{ display: 'grid', gap: '16px', gridTemplateColumns: '1fr', marginBottom: '120px' }}>
            <div style={{ backgroundColor: 'white', borderRadius: '20px', padding: '18px', boxShadow: '0 10px 25px rgba(0,0,0,0.05)' }}>
              ...
              <div style={{ marginTop: '14px', display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                <div style={{ backgroundColor: '#EEF2FF', borderRadius: '14px', padding: '12px', flex: '1 1 120px' }}>
                  <div style={{ fontSize: '12px', color: '#6B7280', marginBottom: '6px' }}>Latest update</div>
                  <div style={{ fontWeight: '700', color: '#111827' }}>{historyEvents[0]?.eventType || 'No data'}</div>
                </div>
                <div style={{ backgroundColor: '#EEF2FF', borderRadius: '14px', padding: '12px', flex: '1 1 120px' }}>
                  <div style={{ fontSize: '12px', color: '#6B7280', marginBottom: '6px' }}>Current zone</div>
                  <div style={{ fontWeight: '700', color: isSafe ? '#16A34A' : '#DC2626' }}>{isSafe ? 'Safe' : 'Alert'}</div>
                </div>
                <div style={{ backgroundColor: '#EEF2FF', borderRadius: '14px', padding: '12px', flex: '1 1 120px' }}>
                  <div style={{ fontSize: '12px', color: '#6B7280', marginBottom: '6px' }}>Battery</div>
                  <div style={{ fontWeight: '700', color: '#111827' }}>{`${childData?.battery || 100}%`}</div>
                </div>
              </div>
            </div>
          </div>
